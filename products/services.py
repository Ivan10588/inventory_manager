...
@transaction.atomic
def списать_товар(equipment_id: int, quantity: int, user_id: int, idempotency_key: str | None = None):
    """
    ...
    if equipment.stock_quantity < quantity:
        raise ValidationError(f"Недостаточно оборудования. Доступно: {equipment.stock_quantity}, запрошено: {quantity}")

    equipment.stock_quantity -= quantity
    equipment.save(update_fields=['stock_quantity'])
